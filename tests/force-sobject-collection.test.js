describe('force-sobject-collection', function() {
    ...
    describe('Offline', function() {
        ...
        var records = [
            ...
            {attributes: {type: 'account'}, Id:"009", Name:"JamesOther"}
        ];

        /* Disable isOnline for this testsuite */
        before(function() {
            isOnline = SFDC.isOnline;
            SFDC.isOnline = function() { return false; }
        });

        after(function() {
            SFDC.isOnline = isOnline;
        });
        ...
        describe('#querytype', function() {
            it('should fetch all data from cache when querytype is not defined', function(done) {
                sobjectColl.fetch();
                sobjectColl.addEventListener('sync', function() {
                    sobjectColl.collection.should.have.length(3);
                    sobjectColl.collection.models[0].id.should.eql('007');
                    sobjectColl.collection.models[1].id.should.eql('008');
                    sobjectColl.collection.models[2].id.should.eql('009');
                    done();
                });
            });

            it('should fetch data from cache using cache query when querytype is cache and query is defined', function(done) {
                sobjectColl.querytype = 'cache';
                sobjectColl.query = navigator.smartstore.buildExactQuerySpec('Id', '007');
                sobjectColl.fetch();
                sobjectColl.addEventListener('sync', function() {
                    sobjectColl.collection.should.have.length(1);
                    sobjectColl.collection.models[0].id.should.eql('007');
